package linda.video.pixel;
	...
	class TRTextureGouraudAlpha extends TriangleRenderer,implements ITriangleRenderer
	{
		public function new()
		{
			super();
						...
						for (yi in ystart...yend)
						{
							...
							for (xi in xstart...xend)
							{
								...
								if (zi > buffer[pos])
								{
									...
									target[pos] = (
		                  					       ((Std.int(alpha * ri) + invAlpha * (bgColor >> 16 & 0xFF)) * (textel >> 16 & 0xFF) >> 16)  << 16 | 
						  					       ((Std.int(alpha * gi) + invAlpha * (bgColor >> 8 & 0xFF))  * (textel >> 8 & 0xFF)  >> 16)  << 8  | 
						  					       ((Std.int(alpha * bi) + invAlpha * (bgColor & 0xFF))       * (textel & 0xFF)       >> 16)
						                          );
								}
								ui += du; vi += dv; zi += dz;
								ri += dr; gi += dg; bi += db;
