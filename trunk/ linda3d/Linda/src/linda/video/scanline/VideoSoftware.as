package linda.video.scanline
{
	...
	public class VideoSoftware extends VideoNull implements IVideoDriver
	{
		...
		private var _tr:TRTemplate;
		private var _trAlpha:TRTemplateAlpha;
		...
		public function VideoSoftware (size : Dimension2D)
		{
			...
			if (size == null) size = new Dimension2D (400, 400);
			screenSize = size;
			//render target
			targetBitmap = new Bitmap ();
			targetBitmap.smoothing = false;
			targetBitmap.cacheAsBitmap = false;
			targetBitmap.bitmapData = new BitmapData (size.width, size.height, false, 0x0);
			renderTarget.addChild (targetBitmap);
			_buffer=new SBuffer();
			_buffer.init(targetBitmap.bitmapData);
		...
		private function init () : void
		{
			...
			//render
			_tr=new TRTemplate(_buffer);
			_trAlpha=new TRTemplateAlpha(_buffer);
			triangleRenderers = new Vector.<ITriangleRenderer>();
			triangleRenderers.push(_tr);
			triangleRenderers.push(_trAlpha);
		...
		{
			primitivesDrawn = 0;
			_buffer.reset(backBuffer,color);
			return true;
		}
		...
		public override function setMaterial (mat : Material) : void
		{
		    ...
		    _tr.type=getTRIndex();
		    _trAlpha.type=_tr.type;
		    if(material.transparenting)
		    {
		    	currentTriangleRenderer=_trAlpha;
		    }else
		    {
		    	currentTriangleRenderer=_tr;
		    }
